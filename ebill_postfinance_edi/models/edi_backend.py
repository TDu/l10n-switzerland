...
class EdiBackend(models.Model):
    ...
    ebill_postfinance_service_id = fields.Many2one("ebill.postfinance.service")
    ...
    def _get_component_usage_candidates(self, exchange_record, key):
        ...
        if not self.ebill_postfinance_service_id or key not in self._webservice_actions:
            return candidates
        return [f"postfinanceservice.{key}"] + candidates
